document.getElementById('shorten-form').addEventListener('submit', async function(event) {
    ...
    // Ambil elemen pesan error yang baru
    const errorMessageDiv = document.getElementById('error-message');

    // Sembunyikan hasil dan pesan error sebelumnya setiap kali tombol ditekan
    resultDiv.classList.add('hidden');
    errorMessageDiv.classList.add('hidden');
    errorMessageDiv.textContent = ''; // Kosongkan teks error
    ...
    try {
        ...
        if (response.ok) {
            ...
            // --- LOGIKA ERROR BARU ---
            // Alih-alih menggunakan alert, kita tampilkan di elemen div
            const errorData = await response.json();
            errorMessageDiv.textContent = `Gagal: ${errorData.error || 'Terjadi kesalahan.'}`;
            errorMessageDiv.classList.remove('hidden');
        }
    } catch (error) {
        // Menangani error jaringan atau fetch
...
document.getElementById('copy-btn').addEventListener('click', function() {
    ...
    // Menggunakan fallback untuk kompatibilitas yang lebih baik
